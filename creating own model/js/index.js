...
machineLearningLib.neuralNetworkFunctions.costFunction = function (X, y, K, thetas, lambda) {
    ...
    for (k = 0; k < K; k++) {
        ...
        let My1 = machineLearningLib.matrixFunctions.executeOnAllElements([y], function (x) { return -1 * (x === k); });
        let My2 = machineLearningLib.matrixFunctions.executeOnAllElements([y], function (x) { return 1 - 1 * (x === k); });
        ...
        let s = machineLearningLib.matrixFunctions.sumMatrix(M);
        console.log(M)
        // console.log(s, 105.7724);
        J += 1 / X.length * s;
       console.log(J, [0.0212, 0.0514, 0.0908, 0.1206, 0.1506, 0.1708, 0.199, 0.2338, 0.2733, 0.2876][k])
    }
